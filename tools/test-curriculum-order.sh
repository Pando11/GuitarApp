...
# NOTE (Windows/MSYS pitfall): keep the sandbox INSIDE the repo tree. Native Windows python3
# cannot resolve an MSYS /tmp path, so mutations would silently fail and the suite would report
# SETUP-BROKEN for every case. Same lesson as the song-gate suite — keep it in-repo.
SANDBOX="$ROOT/.order-sandbox"
rm -rf "$SANDBOX"
