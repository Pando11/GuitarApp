...
| 5A Sage voice: server route | DONE (2026-09-10) | `POST /coach/speak` via fal.ai Kokoro TTS, separate from `/coach` so a voice failure can't touch the text answer. Default voice `af_heart` — same voice as the app's pre-recorded narration. ~$0.02/1000 chars, ~2s synthesis. 117/117 server tests. Independently re-verified with two live round trips (different voices, different real audio) |
| 5B Sage voice: client wiring | DONE (2026-09-11) | Tap-to-play control on both the lesson chat and practice screen, never autoplays. **Bug caught by independent verification before commit:** the control was gating on answer-text-truthiness only, so it mounted next to boilerplate template/fallback text too, indistinguishable from a real answer — reproduced live twice, fixed to require `source === 'model'`. `lesson-runner.js` had zero test coverage before this; now has 20 assertions. Re-verified a second time after the fix, both original failures confirmed gone, real-answer case confirmed still working, both surfaces |
| — SW unhandled rejection on every POST | DONE (2026-09-11) | Found while verifying 5B: `service-worker.js`'s `networkFirst()` tried to cache every POST response, which the Cache API rejects — threw on every real `/coach`/`/coach/speak` call in production (silent, didn't break the response, just console noise). Fixed to only cache GET. Independently reproduced the exact crash pre-fix and its absence post-fix |
