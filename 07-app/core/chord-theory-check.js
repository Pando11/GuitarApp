// chord-theory-check.js â€” DETERMINISTIC chord verification. No human required.
...
// Tones that are REQUIRED â€” if absent the chord is not what its name claims.
...
// Quality tokens we RECOGNIZE. Anything quality-looking that isn't here must FAIL CLOSED â€”
...
// (see the m7b5 entry) â€” that is the half-diminished (m7b5) branch, not fully-dim.
...
const QUALITY_PATTERNS = [
  ...
  // base qualities â€” ordered so min7/maj7 win over bare minor/major
...
function parseChordName(name) {
  ...
  const tokens = bare.split(/[\s,\/\-â€”]+/).filter(Boolean);
  ...
  // cannot drift â€” the pre-commit hook re-proves they stay 1:1.
  // NOTE: dim7 deliberately does NOT include \bm7b5 â€” that is the half-diminished
...
function verifyChord(key, chord) {
  ...
  // 0. STRUCTURAL checks â€” this function must be self-contained because the audit and
  ...
  if (parsed.unknownQuality) {
    return { key, ok: false,
      errors: ['chord name "' + chord.name + '" uses quality "' + parsed.unknownQuality +
               '" which the checker has no recipe for â€” refusing to verify. Add a recipe or rename.'],
      warnings, notes: [] };
  }
  ...
  for (const iv of recipe) {
    ...
    if (!soundedPcs.has(pc)) {
      ...
      if (req.includes(iv)) {
        errors.push('REQUIRED tone ' + label + ' (' + NOTES[pc] + ') is missing â€” this is not a ' + chord.name);
      } else {
        warnings.push('the ' + label + ' (' + NOTES[pc] + ') is omitted (acceptable in a reduced voicing)');
      }
    }
  }
  // 3. bass note check â€” lowest sounding note is usually the root for open chords
  if (sounded.length) {
  ...
  for (const [fg, ds] of Object.entries(byFinger)) {
    ...
    if (frets.length > 1) errors.push('finger ' + fg + ' is on two different frets (' + frets.join(' and ') + ') â€” physically impossible');
  ...
  for (let i = 1; i < sortedByFret.length; i++) {
    if (sortedByFret[i].finger < sortedByFret[i - 1].finger &&
      ...
      warnings.push('finger ' + sortedByFret[i].finger + ' sits behind finger ' +
                    sortedByFret[i - 1].finger + ' â€” awkward but not impossible');
    }
  }
...
export { verifyChord, verifyLesson, parseChordName, NOTES, OPEN_MIDI, QUALITIES, REQUIRED, QUALITY_PATTERNS };
