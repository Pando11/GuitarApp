...
// ATTRIBUTION (boardroom/growth-2026-08): every generated cinematic asset must carry an
// ATTRIBUTION block so installs/subscriptions can be traced back to the world/teacher that
// drove them (free + offline — PocketBase / local ledger; no paid dependency introduced).
// This extends the existing code-enforced license gate into an install/subscription-attribution gate.
export const ATTRIBUTION_FIELDS = ['campaign_source', 'content_id', 'world_id'];
...
export function validateAssetJob(job, teacherRef) {
  ...
  // ATTRIBUTION (boardroom/growth): tag every asset for install/subscription attribution.
  // Additive requirement for NEW jobs; existing T1.assetjob.json carries it (non-breaking).
  if (!isObj(job.attribution)) {
    errors.push('AssetJob.attribution missing (required: campaign_source, content_id, world_id for install/subscription attribution)');
  } else {
    for (const f of ATTRIBUTION_FIELDS) {
      if (typeof job.attribution[f] !== 'string' || !job.attribution[f].length)
        errors.push('AssetJob.attribution.' + f + ' missing/empty (install/subscription attribution tag)');
    }
  }
